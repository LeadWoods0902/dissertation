...
interface CharacterDao{
    ...
    @Insert
    suspend fun insertCharacter(character: CharacterSheet)

    @Delete
    suspend fun deleteCharacter(character:CharacterSheet)

    @Update
    suspend fun updateCharacter(character: CharacterSheet)

    @Transaction
    @Query("SELECT * FROM CharacterSheet WHERE characterID = :searchTerm")
    suspend fun retrieveByID(searchTerm: Int): CharacterSheet?

    @Transaction
    @Query("SELECT * FROM CharacterSheet WHERE name = :name OR (:characterClass IS NULL OR characterClass = :characterClass) OR (:level IS NULL OR level = :level)")
    suspend fun retrieveCharacters(name: String?, characterClass: CharacterClass?, level: Int?): List<CharacterSheet>?
}
